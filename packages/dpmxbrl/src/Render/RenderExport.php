...
use App\Models\HierarchyTableTier;
...
class RenderExport
{
    ...
    public function renderHtml($xbrl, $additional = array(), $lang = 'bs-Latn-BA')
    {
...
//
        ...
        $tableRC =
            $this->axis->searchLabel($this->specification['rend']['path'] . "#" . $tableLabelName, 'http://www.eurofiling.info/xbrl/role/rc-code');

        $tableFI =
            $this->axis->searchLabel($this->specification['rend']['path'] . "#" . $tableLabelName, 'http://www.eurofiling.info/xbrl/role/filing-indicator-code');
        ...
        $this->tableDB = Table::updateOrCreate(
            ['ext_code' => $tableID],
            [
                'orginal_table_code' => $tableName,
                'orginal_table_label' => $labelFullName,
                'table_path' => $additional['table_path'],
                'rc_code' => $tableRC,
                'fi_code' => $tableFI,
                'tc_code' => $tableLabelName,
            ]);
        ...
        if (!is_null($aspectNode)):
            ...
            for ($y = 0; $y < $maxRow; $y++):
                ...
                foreach ($this->col as $col) {
                    ...
                    $additional['c-code'] = $col['rc-code'];
                    $additional['r-code'] = null;
                    $typ = array();
                ...
                case 'xbrli:QNameItemType':
                    ...
                    if (isset($def['presentation'])):
                        ...
                        foreach ($def['presentation'] as $row):
                            $hie = Hierarchy::updateOrCreate(
                                ['hierarchy_code' => $this->axis->getHierKey($def['namespace'], $row['href'])],

                                [
                                    'hierarchy_label_bs_latn_ba' => $row['@content'],
                                    'hierarchy_label_en' => null,
                                    'domain_id' => null,
                                    'concept_id' => null
                                ]);

                            HierarchyTableTier::updateOrCreate(
                                [
                                    'hierarchy_id' => $hie->id,
                                    'table_id' => $this->tableDB->id,
                                    'c_code' => $additional['c-code'],
                                    'r_code' => $additional['r-code'],
                                ],
                                [
                                    'hierarchy_id' => $hie->id,
                                    'table_id' => $this->tableDB->id,
            ...
            if (isset($additional['explicitDimension'])):
                ...
                foreach ($additional['explicitDimension'] as $row):
                    ...
                    if (!empty($row['order'])):
                        ...
                        $hie = Hierarchy::updateOrCreate(
                            ['hierarchy_code' => json_encode($key)],
